...
import { UserFilter, UserFindOneOptions, UserUpdatePayload, UpdateAffected } from '../../../domain/repositories/user.filter';
...
export class UserRepositoryDrizzle extends UserRepository {
  ...
  async findByEmail(email: string, options?: UserFindOneOptions): Promise<User | null> {
    ...
    return rows[0] ? this.toUser(rows[0].user as UserRow, rows[0].role) : null;
  }
